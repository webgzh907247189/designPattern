...
const allData = [...document.getElementsByTagName('tmp')].reduce((result, item) => {
    const val = dayjs('2021-07-01 ' + (item.textContent.replace(/\n/g,'').trim()).padStart(8, '00:') ).unix()

    // 得到从 0点 开始点 unix 时间差
    const aa = val - dayjs('2021-07-01 00:00:00').unix()
    if(!Number.isNaN(val)){
        result += aa;

        let s= dayjs('2021-07-01 00:00:00').unix() + aa;
...
let time = dayjs('2021-07-01 00:00:00').unix() + allData;
