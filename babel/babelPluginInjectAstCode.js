    ...
    visitor: {
      JSXAttribute(astPath, state) {
        ...
        if (jsxIdentifierName === "dangerouslySetInnerHTML") {
          ...
          const jsxIdentifierExpression = astPath
            .get("value")
            .get("expression");
          ...
          for (const jsxPropertyItem of jsxPropertyList) {
            ...
            if (jsxPropertyItemKey === "__html") {
              const jsxPropertyItemValueToString =
                jsxPropertyItemValue.toString();

              jsxPropertyItemValue.replaceWithSourceString(
                `${cusOptions.injectFnName}(${jsxPropertyItemValueToString})`
              );
              ...
              if (isImportAstNode) {
                ...
                if (isImportSpecifier && importPackage === cuspPackageName) {
                  ...
                  for (const itemImportSpecifier of itemImportSpecifiersList) {
                    const itemImportSpecifierName = itemImportSpecifier
                      .get("imported")
                      .toString();
